...
from alfred.vis.image.pose import (
    vis_pose_by_joints,
...
from yolov7.data.custom_datasets import register_ap10k_datasets
import numpy as np

# constants
WINDOW_NAME = "COCO detections"
register_ap10k_datasets()
...
def setup_cfg(args):
    ...
    # cfg.INPUT.MAX_SIZE_TEST = 1280  # 90ms
    # cfg.INPUT.MAX_SIZE_TEST = 608  # 90ms
    # cfg.INPUT.MAX_SIZE_TEST = 512  # 90ms
    cfg.freeze()
    return cfg
...
def vis_res_fast(res, img, class_names, colors, thresh, meta):
    ins = res["instances"]
    bboxes = None
    ...
    if ins.has("pred_keypoints"):
        kps_coords = ins.pred_keypoints
        print(kps_coords.shape)
        try:
            joints = np.array(meta.skeleton)
        except AttributeError:
            logger.info('using coco default 17 joints.')
            joints = np.array(BUILTIN_JOINTS_17_COCO)
        img = vis_pose_by_joints(img, kps_coords.cpu().numpy(), joints, point_size=0, line_width=3)
        # img = vis_pose_coco_17(img, kps_coords.cpu().numpy())

    thickness = 1 if ins.has("pred_bit_masks") else 2
    font_scale = 0.4 if ins.has("pred_bit_masks") else 0.6
...
if __name__ == "__main__":
    ...
    logger.info(f'{cfg.INPUT.MIN_SIZE_TEST}, {cfg.INPUT.MIN_SIZE_TEST}, {cfg.INPUT.MAX_SIZE_TEST}')
    colors = [
        [random.randint(0, 255) for _ in range(3)]
        for _ in range(cfg.MODEL.YOLO.CLASSES)
    ]
    conf_thresh = cfg.MODEL.YOLO.CONF_THRESHOLD
    logger.info(f"confidence thresh: {conf_thresh}")
    ...
    while True:
        ...
        res = predictor(im)
        res = vis_res_fast(res, im, class_names, colors, conf_thresh, metadata)
