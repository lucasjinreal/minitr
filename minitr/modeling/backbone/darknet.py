...
class Darknet(Backbone):
    ...
    def __init__(self, depth, ch_in=3, nf=32, out_features=None, num_classes=None):
        ...
        for i, nb in enumerate(num_blocks):
            ...
            self._output_shape[name] = ShapeSpec(channels=nf, stride=16)
        if num_classes is not None:
            name = "linear"
            self.add_module(
                name,
                nn.Sequential(
                    [nn.AdaptiveAvgPool2d(1), Flatten(), nn.Linear(nf, num_classes)]
                ),
            )
...
def build_darknet_backbone(cfg, input_shape):
    ...
    model = Darknet(depth, input_shape.channels, stem_channels, output_features)
