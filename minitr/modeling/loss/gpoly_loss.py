'''
Apply PolyLoss to generalized loss
'''
import torch.nn as nn
import torch.nn.functional as F

from .loss import weight_reduce_loss


def quality_poly1_loss(
    pred,  # (n, 80)
    ...
    loss += epsilon * (1 - pt).pow(beta + 1)
    loss = weight_reduce_loss(loss, weight, reduction, avg_factor)
    return loss


def distribution_focal_loss(
    pred, label, weight=None, reduction="mean", avg_factor=None, epsilon=-1
):
    disl = label.long()
    disr = disl + 1
    ...
    loss = (
        F.cross_entropy(pred, disl, reduction="none") * wl
...
class QualityPoly1Loss(nn.Module):
    ...
    def forward(
        self, pred, target, score, weight=None, avg_factor=None, reduction_override=None
    ):
        assert reduction_override in (None, "none", "mean", "sum")
        reduction = reduction_override if reduction_override else self.reduction
        if self.use_sigmoid:
            loss_cls = self.loss_weight * quality_poly1_loss(
                pred,
                target,
                score,
                weight,
