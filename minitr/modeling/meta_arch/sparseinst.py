...
def batched_index_select(input, dim, index):
    ...
    return torch.cat(
        torch.chunk(torch.gather(input, dim, index), chunks=index.shape[0], dim=dim),
        dim=0,
    )
...
class SparseInst(nn.Module):
    def __init__(self, cfg):
        ...
        self.normalizer_trans = lambda x: (
            x - self.pixel_mean.unsqueeze(0)
        ) / self.pixel_std.unsqueeze(0)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            if torch.onnx.is_in_onnx_export():
                results = self.inference_onnx(output, batched_inputs, max_shape)
                return results
            else:
