...
class Bottle2neckX(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            inplanes, D * C * scale, kernel_size=1, stride=1, padding=0, bias=False
        )
        ...
        self.conv3 = nn.Conv2d(
            D * C * scale, planes * 4, kernel_size=1, stride=1, padding=0, bias=False
        )
...
def res2next50(pretrained=False, **kwargs):
    ...
    """
    model = Res2NeXt(
        Bottle2neckX, layers=[3, 4, 6, 3], baseWidth=4, cardinality=8, scale=4, **kwargs
    )
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url(model_urls["res2next50"]), strict=False
        )
    return model
