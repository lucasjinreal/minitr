...
class MyDatasetMapper2(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        if (
            self.is_train
            and mosaic_flag == 1
            and mosaic_samples is not None
            and self.enable_aug
        ):
            ...
            # input_dim = (640, 640)
            w = np.random.randint(
                self.mosaic_trans.MOSAIC_WIDTH_RANGE[0],
                self.mosaic_trans.MOSAIC_WIDTH_RANGE[1] + 1,
            )
            h = np.random.randint(
                self.mosaic_trans.MOSAIC_HEIGHT_RANGE[0],
                self.mosaic_trans.MOSAIC_HEIGHT_RANGE[1] + 1,
            )
            if max(w / h, h / w) > 1.2:
                # ratio abnormal
                h = min(h, w)
            ...
            for i in range(self.mosaic_trans.NUM_IMAGES):
                ...
                scale = min(1.0 * input_dim[0] / h0, 1.0 * input_dim[1] / w0)
                ...
                if i == 0:  # top left
                    ...
                    img4 = np.full(
                        (input_dim[0] * 2, input_dim[1] * 2, img.shape[2]),
                        114,
                        dtype=np.uint8,
                    )
