...
def multi_head_rcda_forward(
    ...
    if not torch.jit.is_scripting():
        ...
        if any([type(t) is not Tensor for t in tens_ops]) and has_torch_function(
            ...
            return handle_torch_function(
                ...
                q_row_proj_weight=q_row_proj_weight,
                q_col_proj_weight=q_col_proj_weight,
                k_row_proj_weight=k_row_proj_weight,
                k_col_proj_weight=k_col_proj_weight,
    ...
    assert list(attn_output_weights_row.size()) == [
        bsz * num_heads,
        tgt_len,
        src_len_row,
    ]
    assert list(attn_output_weights_col.size()) == [
        bsz * num_heads,
        tgt_len,
        src_len_col,
    ]
    ...
    if key_padding_mask is not None:
        ...
        attn_output_weights_row = attn_output_weights_row.view(
            bsz, num_heads, tgt_len, src_len_row
        )
        attn_output_weights_col = attn_output_weights_col.view(
            bsz, num_heads, tgt_len, src_len_col
        )

        attn_output_weights_row = attn_output_weights_row.masked_fill(
            mask_row, float("-inf")
        )
        attn_output_weights_col = attn_output_weights_col.masked_fill(
            mask_col, float("-inf")
        )
    ...
    attn_output_weights_col = dropout(
        attn_output_weights_col, p=dropout_p, training=training
    )
    attn_output_weights_row = dropout(
        attn_output_weights_row, p=dropout_p, training=training
    )
    ...
    if efficient_compute:
        if src_len_col < src_len_row:
            ...
            attn_output_row = (
                torch.matmul(
                    attn_output_weights_row,
                    v.permute(0, 2, 1, 3).reshape(b_ein, w_ein, h_ein * c_ein),
                )
            ...
            attn_output = (
                torch.matmul(
                    attn_output_weights_col.permute(1, 0, 2)[:, :, None, :],
                    attn_output_row.permute(2, 0, 1, 3),
                )
                .squeeze(-2)
            ...
            b_ein, h_ein, w_ein, c_ein = v.shape
            attn_output_col = torch.matmul(
                attn_output_weights_col, v.reshape(b_ein, h_ein, w_ein * c_ein)
            ).reshape(b_ein, q_ein, w_ein, c_ein)
            attn_output = (
    ...
    if need_weights:
        return attn_output, torch.einsum(
            "bqw,bqh->qbhw", attn_output_weights_row, attn_output_weights_col
        ).reshape(tgt_len, bsz, num_heads, src_len_col, src_len_row).mean(2)
    else:
