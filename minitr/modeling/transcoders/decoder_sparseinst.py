...
class BaseIAMDecoder(nn.Module):
    ...
    def forward(self, features):
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            pred_masks = torch.bmm(
                pred_kernel, mask_features.view(sh[0], sh[1], sh[2] * sh[3])
            ).view(sh[0], N, sh[2], sh[3])
        else:
...
class GroupInstanceBranch(nn.Module):
    ...
    def forward(self, features):
        ...
        d4 = N // 4
        inst_features = (
            inst_features.reshape(B, 4, d4, -1).transpose(1, 2).reshape(B, d4, -1)
        )
